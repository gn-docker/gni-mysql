...
{
  StartMySQL
  ADMIN=${GNI_DATABASE_ADMIN_USER}
  ADMIN_PASS=${GNI_DATABASE_ADMIN_PASSWORD}
  GNI_USER=${GNI_DATABASE_USER}
  GNI_USER_PASS=${GNI_DATABASE_PASSWORD}

  echo "========================================================================" >> ${LOG}
  echo "Creating '${ADMIN}' user ..." >> ${LOG}
  mysql -uroot -e "CREATE USER '${ADMIN}'@'%' IDENTIFIED BY '${ADMIN_PASS}'"
  mysql -uroot -e "GRANT ALL PRIVILEGES ON *.* TO '${ADMIN}'@'%' WITH GRANT OPTION"
  echo "Creating ${GNI_USER} user ..." >> ${LOG}
  mysql -uroot -e "CREATE USER '${GNI_USER}'@'%' IDENTIFIED BY '${GNI_USER_PASS}'"
  mysql -uroot -e "GRANT ALL PRIVILEGES ON *.* TO '${GNI_USER}'@'%' WITH GRANT OPTION"
  mysql -uroot -e "GRANT ALL PRIVILEGES ON *.* TO '${GNI_USER}'@'%' WITH GRANT OPTION"
